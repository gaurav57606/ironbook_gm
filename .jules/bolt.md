...
## 2024-05-28 - [Performance Optimization: Fast Calendar Day Calculation]
**Learning:** Allocating local `DateTime` objects inside loops in Dart is extremely slow due to OS timezone lookups. When calculating calendar day differences (e.g., `getDaysRemaining`), using `DateTime.difference().inDays` can be a hidden bottleneck, especially when called frequently during list mapping/filtering.
**Action:** Use `DateTime.utc(year, month, day).millisecondsSinceEpoch` and integer division (`~/ 86400000`) instead of `expiry.difference(today).inDays`. This is ~80x faster and prevents Daylight Saving Time (DST) truncation bugs.
