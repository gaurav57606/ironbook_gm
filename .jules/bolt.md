...
## 2024-05-18 - Single-Pass Filter in State Providers
**Learning:** Chaining multiple `.where().toList()` filter operations (e.g., sequentially applying search queries followed by status tabs) causes redundant O(N) list iterations and unnecessary intermediate memory allocations in Dart/Flutter. This overhead is particularly damaging within `Provider` state rebuilds triggered frequently by search inputs or clock ticks.
**Action:** Replace filter chains with a single-pass `for` loop to evaluate all criteria simultaneously. Benchmarks typically show substantial improvements (e.g., ~70% speedup).
