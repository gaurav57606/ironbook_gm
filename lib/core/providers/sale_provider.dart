...
class SaleNotifier extends StateNotifier<List<Sale>> {
  ...
  Future<void> _reconcileSales() async {
    ...
    debugPrint(
        '[STATE] SaleNotifier: Reconciling ${missingEvents.length} missing sales');

    final parsedSales = await Future.wait(missingEvents.map((e) async {
      final saleId = e.payload['saleId'] as String;
      return Sale.fromPayload(saleId, e.payload, e.deviceTimestamp);
    }));

    await _saleRepo.upsertSales(parsedSales.toList());
