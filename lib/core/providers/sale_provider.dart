...
class SaleNotifier extends StateNotifier<List<Sale>> {
  ...
  Future<void> _reconcileSales() async {
    // OPTIMIZATION: Fix N+1 query problem during startup reconciliation.
    // Previously, this looped over all events and individually queried `getSale` and executed `upsertSale`.
    // Now, we retrieve all IDs in O(1), filter the events, and perform a single batch `upsertSales`.
    ...
    final missingEvents = saleEvents.where((e) {
      ...
      return saleId != null && !existingSaleIds.contains(saleId);
    }).toList();

    if (missingEvents.isEmpty) return;

    debugPrint(
        '[DB] SaleNotifier: Reconciling ${missingEvents.length} missing sales in batch');

    final List<Sale> salesToInsert = [];
    ...
    await _saleRepo.upsertSales(salesToInsert);
