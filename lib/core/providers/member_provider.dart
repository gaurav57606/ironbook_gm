...
final filteredMembersProvider = Provider<List<MemberSnapshot>>((ref) {
  ...
  // [PERF OPTIMIZATION]
  // Used a single-pass loop instead of chained `.where().toList()` operations.
  // This reduces the computational complexity from O(k*N) to O(N) and prevents
  // allocating redundant intermediate lists in memory.
  for (final m in members) {
