...
final filteredMembersProvider = Provider<List<MemberSnapshot>>((ref) {
  ...
  // Performance Optimization: Replaced chained .where().toList() filter calls with a single-pass loop.
  // This avoids O(N) list iterations and unnecessary intermediate memory allocations for each filter condition.
  for (final m in members) {
    if (query.isNotEmpty) {
