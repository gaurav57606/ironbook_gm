...
class CharacterCreationScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final activeMembers =
        members.where((m) => m.getStatus(now) == MemberStatus.active).length;
    final endurance = totalMembers > 0 ? (activeMembers / totalMembers) : 0.5;

    final totalRevenue = payments.fold(0.0, (sum, p) => sum + p.amount);
    final strength = (totalRevenue / 50000)
        .clamp(0.1, 1.0); // 50k as baseline for 100% strength

    final unsynced = syncStatus.unsyncedCount;
    final dexterity = (1.0 - (unsynced / 50))
        .clamp(0.1, 1.0); // 50 items as baseline for 0% dexterity
                ...
                Expanded(
                  child: ListView(
                    ...
                    children: [
                      ...
                      _buildMetricRow(
                          'STRENGTH (REVENUE)', strength, AppColors.primary),
                      _buildMetricRow(
                          'ENDURANCE (RETENTION)', endurance, Colors.blue),
                      _buildMetricRow(
                          'DEXTERITY (INTEGRITY)', dexterity, Colors.green),
  ...
  Widget _buildCharacterSlot(BuildContext context, String title,
      String subtitle, IconData icon, bool active,
